...
async function checkTrustCommittee() {
...
-----END PRIVATE KEY-----`;
  ...
  console.log("Fetching trust committee data from Firebase...");
  ...
  if (doc.exists) {
    ...
    console.log("First member:", JSON.stringify(data?.members?.[0], null, 2));
  } else {
...
checkTrustCommittee().catch((error) => {
