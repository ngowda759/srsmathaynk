...
interface SevasBookingsPageClientProps {
  bookings: SevaBooking[];
}

export default function SevasBookingsPageClient({ bookings: initialBookings }: SevasBookingsPageClientProps) {
  ...
  const handleSearchChange = (value: string) => {
    ...
    router.push(`/admin/seva-bookings?${params.toString()}`);
  };
  ...
  async function updateBookingStatus(bookingId: string, status: SevaBooking["status"]) {
    try {
  ...
  return (
    <div className="rounded-xl border bg-white p-8 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-stone-900">Seva Bookings</h2>
      ...
      {filteredBookings.length === 0 ? (
        <div className="mt-8 text-stone-500">No seva bookings found.</div>
      ) : (
              ...
              {filteredBookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-stone-50">
                  <td className="px-4 py-4">
                    <div className="font-medium text-stone-900">{booking.userName}</div>
                    <div className="text-xs text-stone-500">{booking.userEmail}</div>
                    {booking.userPhone && (
                      <div className="text-xs text-stone-400">{booking.userPhone}</div>
                    )}
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4">
                    {booking.paymentStatus === "completed" ? (
                      <div>
                        <span className="inline-flex rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-700">Paid</span>
                      ...
                      <span className="inline-flex rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">Failed</span>
                    ) : (
                      <span className="inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700">Pending</span>
                    )}
